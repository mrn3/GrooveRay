...
export default function Stations() {
  ...
  const buildParams = useCallback(
    (overrides = {}) => {
      ...
      const rc = minRatingCommunity.trim() ? parseFloat(minRatingCommunity, 10) : null;
    ...
    <div>
      ...
      <div className="mb-4 space-y-3 rounded-xl border border-groove-700 bg-groove-900/50 p-4">
        ...
        <div>
          ...
          >
            {filtersExpanded ? '▼ Less filters' : '▶ More filters (ratings)'}
          </button>
          {filtersExpanded && (
            ...
            >
              ...
              <span className="flex-shrink-0 text-xs text-gray-400" title="Currently listening">
                <span className="flex items-center gap-1">
                  ...
                  {(station.listener_count ?? 0) > 0 ? station.listener_count : '—'}
                </span>
              </span>
                ...
                {station.community_rating_count > 0 ? (
                  <span className="flex items-center gap-1">
                    <span className="text-amber-400">
                      {Number(station.community_avg_rating).toFixed(1)} ★
                    </span>
                    <span className="text-gray-500">({station.community_rating_count})</span>
                  </span>
                ) : (
                  '—'
                )}
